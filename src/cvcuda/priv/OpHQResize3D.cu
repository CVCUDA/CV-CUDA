...
class HQResizeImpl3D final : public IHQResizeImpl
{
public:
    ...
    {
        CVCUDA_NVTX_RANGE("cvcuda::hq_resize::HQResizeImpl3D::operator()[Tensor]");
        HQResizeRun<3> resize(m_filtersFactory);
        resize(stream, ws, src, dst, minInterpolation, magInterpolation, antialias, roi);
    }

    void operator()(cudaStream_t, const cvcuda::Workspace &, const nvcv::ImageBatchVarShape &,
                    const nvcv::ImageBatchVarShape &, const NVCVInterpolationType, const NVCVInterpolationType, bool,
                    const HQResizeRoisF) override
    {
        CVCUDA_NVTX_RANGE("cvcuda::hq_resize::HQResizeImpl3D::operator()[ImageBatchVarShape]");
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT,
                              "ImageBatchVarShape is only supported for 2D resize.");
    }
    ...
    {
        CVCUDA_NVTX_RANGE("cvcuda::hq_resize::HQResizeImpl3D::operator()[TensorBatch]");
        HQResizeRun<3> resize(m_filtersFactory);
...
std::unique_ptr<IHQResizeImpl> makeImpl3D()
{
    return std::make_unique<HQResizeImpl3D>();
}
