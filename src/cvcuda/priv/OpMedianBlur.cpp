...
std::unique_ptr<legacy::MedianBlur> MedianBlur::CreateLegacyOp(int)
{
    legacy::DataShape maxIn;
    legacy::DataShape maxOut;
    return std::make_unique<legacy::MedianBlur>(maxIn, maxOut);
}
...
MedianBlur::MedianBlur(const int maxVarShapeBatchSize)
    ...
    : m_legacyOpVarShape([maxVarShapeBatchSize](int)
                         { return std::make_unique<legacy::MedianBlurVarShape>(maxVarShapeBatchSize); })
{
}
...
{
    CVCUDA_NVTX_RANGE("cvcuda::MedianBlur::operator()[Tensor]");
    ...
    NVCV_CHECK_THROW(m_legacyOp.get().infer(*inData, *outData, ksize, stream));
}
...
{
    CVCUDA_NVTX_RANGE("cvcuda::MedianBlur::operator()[ImageBatchVarShape]");
    ...
    {
        throw nvcv::Exception(nvcv::Status::ERROR_INVALID_ARGUMENT, "ksize must be a tensor");
    }

    NVCV_CHECK_THROW(m_legacyOpVarShape.get().infer(*inData, *outData, *ksizeData, stream));
}
