 ...
 *
 * CUDA built-in vector types are also called compound types.  The number of components in the metastruct
 ...
 *
 * Set all elements to the value \p x passed as argument.  For instance, an int3 can have all its elements
 ...
 *
 * Unfortunately typeid().name() in C/C++ typeinfo yields different names depending on the platform.  This
