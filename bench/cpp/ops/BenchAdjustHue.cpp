...
#include "ops/generated/BenchAdjustHueConfig.hpp"
...
// Hue factor is irrelevant to throughput; use a fixed mid-range value.
static constexpr double kHue = 0.25;

template<typename T>
inline void adjusthue(nvbench::state &state, nvbench::type_list<T>)
try
{
    ...
    {
        state.skip("AdjustHue benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts");
        return;
    }
    ...
    {
        state.skip("Fake-planar (NCHW_FAKE) adjusthue benchmark is tensor-only");
        return;
    }
    ...
    // AdjustHue preserves size, so src and dst hold the same number of bytes.
    ...
    if (isFakePlanar) // tensor-only: planar->interleaved->adjust->interleaved->planar
    {
        ...
        benchutils::warmup_and_exec(state, BENCH_ADJUSTHUE_WARMUP_ITERATIONS,
            [&op, &reformatOp, &src, &interSrc, &interDst, &dst](cudaStream_t s) {
                reformatOp(s, src, interSrc);          // NCHW -> NHWC
                op(s, interSrc, interDst, kHue);       // interleaved adjust
    ...
    else if (inputKind == benchutils::InputKind::Tensor)
    {
        ...
        benchutils::warmup_and_exec(state, BENCH_ADJUSTHUE_WARMUP_ITERATIONS,
            [&op, &src, &dst](cudaStream_t s) { op(s, src, dst, kHue); });
    }
...
NVBENCH_BENCH_TYPES(adjusthue, NVBENCH_TYPE_AXES(BENCH_ADJUSTHUE_TYPES))
BENCH_ADJUSTHUE_AXES;
