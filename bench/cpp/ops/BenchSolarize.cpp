...
#include "ops/generated/BenchSolarizeConfig.hpp"
...
template<typename T>
inline void solarize(nvbench::state &state, nvbench::type_list<T>)
try
{
    ...
    {
        state.skip("Solarize benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts");
        return;
    }
    // NCHW_FAKE ("fake planar") is a tensor-only comparison path: planar data is reformatted to
    // interleaved, solarized with the interleaved kernel, and reformatted back — all timed
    ...
    {
        state.skip("Fake-planar (NCHW_FAKE) solarize benchmark is tensor-only");
        return;
    }
    ...
    // Solarize preserves size, so src and dst hold the same number of bytes.
    const long bytes = static_cast<long>(shape.x) * shape.y * shape.z * sizeof(T);
    if (isFakePlanar)
    {
        // reformat(NCHW->NHWC) + solarize + reformat(NHWC->NCHW): reads src + interleaved src + dst,
    ...
    // clang-format off
    if (isFakePlanar) // tensor-only: planar->interleaved->solarize->interleaved->planar
    {
        ...
        benchutils::warmup_and_exec(state, BENCH_SOLARIZE_WARMUP_ITERATIONS,
            [&op, &reformatOp, &src, &interSrc, &interDst, &dst, threshold](cudaStream_t s) {
                reformatOp(s, src, interSrc);            // NCHW -> NHWC
                op(s, interSrc, interDst, threshold);    // interleaved solarize
    ...
    else if (inputKind == benchutils::InputKind::Tensor)
    {
        ...
        benchutils::warmup_and_exec(state, BENCH_SOLARIZE_WARMUP_ITERATIONS,
            [&op, &src, &dst, threshold](cudaStream_t s) { op(s, src, dst, threshold); });
    }
...
NVBENCH_BENCH_TYPES(solarize, NVBENCH_TYPE_AXES(BENCH_SOLARIZE_TYPES))
BENCH_SOLARIZE_AXES;
