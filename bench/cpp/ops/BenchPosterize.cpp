...
#include "ops/generated/BenchPosterizeConfig.hpp"
...
template<typename T>
inline void posterize(nvbench::state &state, nvbench::type_list<T>)
try
{
    ...
    // bits is irrelevant to throughput (a single bitwise-and per element); use a fixed mid-range value.
    // constexpr so it needs no lambda capture (it is a constant expression).
    ...
    {
        state.skip("Posterize benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts");
        return;
    }
    // NCHW_FAKE ("fake planar") is a tensor-only comparison path: planar data is reformatted to
    // interleaved, posterized with the interleaved kernel, and reformatted back — all timed
    // together — so the native planar path (NCHW) can be shown to beat the naive
    // convert->op->convert pipeline.
    if (isFakePlanar && inputKind == benchutils::InputKind::VarShape)
    {
        state.skip("Fake-planar (NCHW_FAKE) posterize benchmark is tensor-only");
        return;
    }
    ...
    // Posterize preserves size, so src and dst hold the same number of bytes.
    const long bytes = static_cast<long>(shape.x) * shape.y * shape.z * sizeof(T);
    if (isFakePlanar)
    {
        // reformat(NCHW->NHWC) + posterize + reformat(NHWC->NCHW): reads src + interleaved src + dst,
    ...
    // clang-format off
    if (isFakePlanar) // tensor-only: planar->interleaved->posterize->interleaved->planar
    {
        ...
        benchutils::warmup_and_exec(state, BENCH_POSTERIZE_WARMUP_ITERATIONS,
            [&op, &reformatOp, &src, &interSrc, &interDst, &dst](cudaStream_t s) {
                reformatOp(s, src, interSrc);       // NCHW -> NHWC
                op(s, interSrc, interDst, bits);    // interleaved posterize
    ...
    else if (inputKind == benchutils::InputKind::Tensor)
    {
        ...
        benchutils::warmup_and_exec(state, BENCH_POSTERIZE_WARMUP_ITERATIONS,
            [&op, &src, &dst](cudaStream_t s) { op(s, src, dst, bits); });
    }
...
NVBENCH_BENCH_TYPES(posterize, NVBENCH_TYPE_AXES(BENCH_POSTERIZE_TYPES))
BENCH_POSTERIZE_AXES;
