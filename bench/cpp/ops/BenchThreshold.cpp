...
#include "ops/generated/BenchThresholdConfig.hpp"
...
try
{
    ...
    {
        state.skip("Threshold benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts");
        return;
    }
    if (isFakePlanar && inputKind == benchutils::InputKind::VarShape)
    {
        state.skip("Fake-planar (NCHW_FAKE) Threshold benchmark is tensor-only");
        return;
    }
    if (isPlanar && inputKind == benchutils::InputKind::VarShape && ch == 1)
    {
        state.skip("Single-channel varshape Threshold has no distinct planar image layout");
        return;
    }

    uint32_t threshType = NVCV_THRESH_BINARY | (std::is_same_v<T, uint8_t> && ch == 1 ? NVCV_THRESH_OTSU : 0);
    ...
    // Default-range LcgValues<double>() so the GPU LCG fast path produces
    // float64 [-1, +1] bytes identical to the Python bench's
    // create_tensor(..., dtype="float64", fill_mode="lcg").
    benchutils::FillTensor<double>(thresh, benchutils::LcgValues<double>());
    benchutils::FillTensor<double>(maxval, benchutils::LcgValues<double>());

    if (isFakePlanar) // tensor-only: planar->interleaved->threshold->interleaved->planar
    {
        ...
        benchutils::warmup_and_exec(state, BENCH_THRESHOLD_WARMUP_ITERATIONS,
            [&op, &reformatOp, &src, &interSrc, &interDst, &dst, &thresh, &maxval](cudaStream_t s) {
                reformatOp(s, src, interSrc);
                op(s, interSrc, interDst, thresh, maxval);
    ...
    else if (inputKind == benchutils::InputKind::Tensor)
    {
        ...
        benchutils::warmup_and_exec(state, BENCH_THRESHOLD_WARMUP_ITERATIONS,
            [&op, &src, &dst, &thresh, &maxval](cudaStream_t s) { op(s, src, dst, thresh, maxval); });
    }
...
NVBENCH_BENCH_TYPES(threshold, NVBENCH_TYPE_AXES(BENCH_THRESHOLD_TYPES))
