...
#include "ops/generated/BenchAverageBlurConfig.hpp"
...
try
{
    ...
    {
        state.skip("AverageBlur benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts");
        return;
    }
    ...
    if (isFakePlanar) // tensor-only: planar->interleaved->averageblur->interleaved->planar
    {
        ...
        benchutils::warmup_and_exec(state, BENCH_AVERAGEBLUR_WARMUP_ITERATIONS,
            [&op, &reformatOp, &src, &interSrc, &interDst, &dst, &kernelSize2d, &kernelAnchor, &borderType](cudaStream_t s) {
                reformatOp(s, src, interSrc);
                op(s, interSrc, interDst, kernelSize2d, kernelAnchor, borderType);
    ...
    else if (inputKind == benchutils::InputKind::Tensor)
    {
        ...
        benchutils::warmup_and_exec(state, BENCH_AVERAGEBLUR_WARMUP_ITERATIONS,
            [&op, &src, &dst, &kernelSize2d, &kernelAnchor, &borderType](cudaStream_t s) {
                op(s, src, dst, kernelSize2d, kernelAnchor, borderType);
            });
    }
    else // zero and positive var shape means use ImageBatchVarShape
    {
        ...
        benchutils::FillTensor<int2>(kernelSizeTensor, [&kernelSize](const long4_16a &){ return kernelSize; });
        benchutils::FillTensor<int2>(kernelAnchorTensor, [&kernelAnchor](const long4_16a &){ return kernelAnchor; });

        benchutils::warmup_and_exec(state, BENCH_AVERAGEBLUR_WARMUP_ITERATIONS,
            [&op, &src, &dst, &kernelSizeTensor, &kernelAnchorTensor, &borderType](cudaStream_t s) {
                op(s, src, dst, kernelSizeTensor, kernelAnchorTensor, borderType);
            });
    }
}
...
NVBENCH_BENCH_TYPES(averageblur, NVBENCH_TYPE_AXES(BENCH_AVERAGEBLUR_TYPES))
BENCH_AVERAGEBLUR_AXES;
