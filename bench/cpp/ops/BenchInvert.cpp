...
#include "ops/generated/BenchInvertConfig.hpp"
...
template<typename T>
inline void invert(nvbench::state &state, nvbench::type_list<T>)
try
{
    ...
    {
        state.skip("Invert benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts");
        return;
    }
    // NCHW_FAKE ("fake planar") is a tensor-only comparison path: planar data is reformatted to
    // interleaved, inverted with the interleaved kernel, and reformatted back — all timed together —
    ...
    {
        state.skip("Fake-planar (NCHW_FAKE) invert benchmark is tensor-only");
        return;
    }
    ...
    // Invert preserves size, so src and dst hold the same number of bytes.
    const long bytes = static_cast<long>(shape.x) * shape.y * shape.z * sizeof(T);
    if (isFakePlanar)
    {
        // reformat(NCHW->NHWC) + invert + reformat(NHWC->NCHW): reads src + interleaved src + dst,
    ...
    // clang-format off
    if (isFakePlanar) // tensor-only: planar->interleaved->invert->interleaved->planar
    {
        ...
        benchutils::warmup_and_exec(state, BENCH_INVERT_WARMUP_ITERATIONS,
            [&op, &reformatOp, &src, &interSrc, &interDst, &dst](cudaStream_t s) {
                reformatOp(s, src, interSrc);   // NCHW -> NHWC
                op(s, interSrc, interDst);      // interleaved invert
...
NVBENCH_BENCH_TYPES(invert, NVBENCH_TYPE_AXES(BENCH_INVERT_TYPES))
BENCH_INVERT_AXES;
