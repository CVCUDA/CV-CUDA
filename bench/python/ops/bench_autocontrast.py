...
"""CV-CUDA Auto Contrast operator benchmark - Python equivalent of BenchAutoContrast.cpp"""
...
def autocontrast(state):
    """Auto Contrast operator benchmark matching C++ BenchAutoContrast.cpp"""
    ...
    if layout not in ("NHWC", "NCHW", "NCHW_FAKE"):
        state.skip(
            "AutoContrast benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts"
        )
        return None
    ...
    if is_fake_planar:
        ...
        def run_fake(launch):
            ...
            cvcuda.autocontrast_into(inter_dst, inter_src, stream=stream)
    ...
    if input_kind == "Tensor":
        ...
        src_batch = cvcuda.ImageBatchVarShape(N)
        dst_batch = cvcuda.ImageBatchVarShape(N)
    ...
    def run(launch):
        cvcuda.autocontrast_into(dst, src, stream=get_stream(launch))

    return run


if __name__ == "__main__":
    run_benchmark("autocontrast", autocontrast)
