...
"""CV-CUDA BrightnessContrast operator benchmark - Python equivalent of BenchBrightnessContrast.cpp"""
...
def brightnesscontrast(state):
    """BrightnessContrast operator benchmark matching C++ BenchBrightnessContrast.cpp"""
    ...
    if layout not in ("NHWC", "NCHW", "NCHW_FAKE"):
        state.skip(
            "BrightnessContrast benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts"
        )
        return None
    ...
    if is_fake_planar and input_kind == "VarShape":
        state.skip(
            "Fake-planar (NCHW_FAKE) BrightnessContrast benchmark is tensor-only"
        )
        return None
    if is_planar and input_kind == "VarShape" and dtype_str == "uchar4":
        state.skip("uchar4 planar ImageBatchVarShape is not supported")
        return None
    ...
    arg_bytes = N * 4 * 4
    if is_fake_planar:
        state.add_global_memory_reads(3 * bytes_ + arg_bytes)
        state.add_global_memory_writes(3 * bytes_)
    else:
        state.add_global_memory_reads(bytes_ + arg_bytes)
        state.add_global_memory_writes(bytes_)

    brightness = create_tensor(
        (N,), cvcuda.Type.F32, device_id, layout="N", fill_mode="lcg"
    )
    contrast = create_tensor(
        (N,), cvcuda.Type.F32, device_id, layout="N", fill_mode="lcg"
    )
    brightness_shift = create_tensor(
        (N,), cvcuda.Type.F32, device_id, layout="N", fill_mode="lcg"
    )
    contrast_center = create_tensor(
        (N,), cvcuda.Type.F32, device_id, layout="N", fill_mode="lcg"
    )
    ...
    if is_fake_planar:
        ...
        def run_fake(launch):
            ...
            cvcuda.brightness_contrast_into(
                inter_dst,
                inter_src,
                brightness,
                contrast,
    ...
    if input_kind == "Tensor":  # Tensor mode
        tshape = (N, num_channels, H, W) if is_planar else (N, H, W, num_channels)
        src = create_tensor(
            tshape,
            dtype,
            device_id,
...
if __name__ == "__main__":
    run_benchmark("brightnesscontrast", brightnesscontrast)
