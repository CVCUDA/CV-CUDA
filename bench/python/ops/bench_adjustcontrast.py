...
"""CV-CUDA Adjust Contrast operator benchmark - Python equivalent of BenchAdjustContrast.cpp"""
...
# Contrast factor is irrelevant to throughput; use a fixed value > 1 that clamps.
CONTRAST_FACTOR = 1.5


def adjustcontrast(state):
    """Adjust Contrast operator benchmark matching C++ BenchAdjustContrast.cpp"""
    ...
    if layout not in ("NHWC", "NCHW", "NCHW_FAKE"):
        state.skip(
            "AdjustContrast benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts"
        )
        return None
    ...
    # AdjustContrast reads the image twice (mean reduction + blend) and writes it once.
    dtype_size = get_dtype_size(dtype_str)
    bytes_ = N * H * W * dtype_size
    if is_fake_planar:
        state.add_global_memory_reads(4 * bytes_)
        state.add_global_memory_writes(3 * bytes_)
    else:
        state.add_global_memory_reads(2 * bytes_)
    ...
    if is_fake_planar:
        ...
        def run_fake(launch):
            ...
            cvcuda.adjust_contrast_into(
                inter_dst, inter_src, CONTRAST_FACTOR, stream=stream
            )
    ...
    def run(launch):
        cvcuda.adjust_contrast_into(
            dst, src, CONTRAST_FACTOR, stream=get_stream(launch)
        )
...
if __name__ == "__main__":
    run_benchmark("adjustcontrast", adjustcontrast)
