...
"""CV-CUDA Adjust Saturation operator benchmark - Python equivalent of BenchAdjustSaturation.cpp"""
...
def adjustsaturation(state):
    """Adjust Saturation operator benchmark matching C++ BenchAdjustSaturation.cpp"""
    ...
    if layout not in ("NHWC", "NCHW", "NCHW_FAKE"):
        state.skip(
            "AdjustSaturation benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts"
        )
        return None
    ...
    # AdjustSaturation preserves size.
    ...
    if is_fake_planar:
        ...
        def run_fake(launch):
            ...
            cvcuda.adjust_saturation_into(
                inter_dst, inter_src, SATURATION, stream=stream
            )
    ...
    def run(launch):
        cvcuda.adjust_saturation_into(dst, src, SATURATION, stream=get_stream(launch))

    return run


if __name__ == "__main__":
    run_benchmark("adjustsaturation", adjustsaturation)
