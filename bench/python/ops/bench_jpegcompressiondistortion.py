...
"""CV-CUDA Jpeg Compression Distortion operator benchmark - Python equivalent of
BenchJpegCompressionDistortion.cpp"""
...
# Quality only scales the quantization tables, so it does not affect throughput; use a fixed
# mid-range value.
QUALITY = 50


def jpegcompressiondistortion(state):
    """Jpeg Compression Distortion benchmark matching C++ BenchJpegCompressionDistortion.cpp"""
    ...
    if layout not in ("NHWC", "NCHW", "NCHW_FAKE"):
        state.skip(
            "JpegCompressionDistortion benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts"
        )
        return None
    ...
    if is_fake_planar and input_kind == "VarShape":
        state.skip(
            "Fake-planar (NCHW_FAKE) jpegcompressiondistortion benchmark is tensor-only"
        )
        return None

    # JpegCompressionDistortion preserves size.
    dtype_size = get_dtype_size(dtype_str)
    bytes_ = N * H * W * dtype_size
    if is_fake_planar:
        # reformat(NCHW->NHWC) + distort + reformat(NHWC->NCHW)
    ...
    if is_fake_planar:
        ...
        def run_fake(launch):
            ...
            cvcuda.jpeg_compression_distortion_into(
                inter_dst, inter_src, QUALITY, stream=stream
            )
    ...
    def run(launch):
        cvcuda.jpeg_compression_distortion_into(
            dst, src, QUALITY, stream=get_stream(launch)
        )
...
if __name__ == "__main__":
    run_benchmark("jpegcompressiondistortion", jpegcompressiondistortion)
