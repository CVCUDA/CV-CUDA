...
"""CV-CUDA Posterize operator benchmark - Python equivalent of BenchPosterize.cpp"""
...
# bits is irrelevant to throughput; use a fixed mid-range value.
BITS = 4


def posterize(state):
    """Posterize operator benchmark matching C++ BenchPosterize.cpp"""
    ...
    if layout not in ("NHWC", "NCHW", "NCHW_FAKE"):
        state.skip(
            "Posterize benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts"
        )
        return None
    ...
    # Posterize preserves size.
    dtype_size = get_dtype_size(dtype_str)
    bytes_ = N * H * W * dtype_size
    if is_fake_planar:
        # reformat(NCHW->NHWC) + posterize + reformat(NHWC->NCHW)
    ...
    if is_fake_planar:
        ...
        def run_fake(launch):
            ...
            cvcuda.posterize_into(inter_dst, inter_src, BITS, stream=stream)
    ...
    def run(launch):
        cvcuda.posterize_into(dst, src, BITS, stream=get_stream(launch))

    return run


if __name__ == "__main__":
    run_benchmark("posterize", posterize)
