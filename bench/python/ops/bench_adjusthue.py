...
"""CV-CUDA Adjust Hue operator benchmark - Python equivalent of BenchAdjustHue.cpp"""
...
def adjusthue(state):
    """Adjust Hue operator benchmark matching C++ BenchAdjustHue.cpp"""
    ...
    if layout not in ("NHWC", "NCHW", "NCHW_FAKE"):
        state.skip(
            "AdjustHue benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts"
        )
        return None
    ...
    # AdjustHue preserves size.
    dtype_size = get_dtype_size(dtype_str)
    bytes_ = N * H * W * dtype_size
    if is_fake_planar:
        # reformat(NCHW->NHWC) + adjust + reformat(NHWC->NCHW)
    ...
    if is_fake_planar:
        ...
        def run_fake(launch):
            ...
            cvcuda.adjust_hue_into(inter_dst, inter_src, HUE, stream=stream)
    ...
    def run(launch):
        cvcuda.adjust_hue_into(dst, src, HUE, stream=get_stream(launch))

    return run


if __name__ == "__main__":
    run_benchmark("adjusthue", adjusthue)
