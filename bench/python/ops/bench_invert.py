...
"""CV-CUDA Invert operator benchmark - Python equivalent of BenchInvert.cpp"""
...
def invert(state):
    """Invert operator benchmark matching C++ BenchInvert.cpp"""
    ...
    # Invert preserves size.
    dtype_size = get_dtype_size(dtype_str)
    bytes_ = N * H * W * dtype_size
    if is_fake_planar:
        # reformat(NCHW->NHWC) + invert + reformat(NHWC->NCHW)
    ...
    if is_fake_planar:
        ...
        def run_fake(launch):
            ...
            cvcuda.invert_into(inter_dst, inter_src, stream=stream)
    ...
    def run(launch):
        cvcuda.invert_into(dst, src, stream=get_stream(launch))

    return run


if __name__ == "__main__":
    run_benchmark("invert", invert)
