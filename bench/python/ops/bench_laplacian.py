...
"""CV-CUDA Laplacian operator benchmark - Python equivalent of BenchLaplacian.cpp"""
...
def laplacian(state):
    """Laplacian operator benchmark matching C++ BenchLaplacian.cpp"""
    ...
    ksize_scalar = int(state.get_int64("ksize"))
    scale_scalar = float(state.get_float64("scale"))
    try:
    ...
    if layout not in ("NHWC", "NCHW", "NCHW_FAKE"):
        state.skip(
            "Laplacian benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts"
        )
        return None
    ...
    def do_laplacian(dst, src, stream):
        cvcuda.laplacian_into(
            dst,
            src,
            ksize=ksize_scalar,
            scale=scale_scalar,
    ...
    if is_fake_planar:
        ...
        def run_fake(launch):
            ...
            do_laplacian(inter_dst, inter_src, stream)
    ...
    if input_kind == "Tensor":
        ...
        def run(launch):
            do_laplacian(dst, src, get_stream(launch))
        ...
        ksize = create_tensor(
            (N,), cvcuda.Type.S32, device_id, layout="N", fill_mode=ksize_scalar
        )
        scale = create_tensor(
            (N,), cvcuda.Type.F32, device_id, layout="N", fill_mode=scale_scalar
        )

        def run(launch):
            cvcuda.laplacian_into(
                dst, src, ksize, scale, border=border, stream=get_stream(launch)
            )
...
if __name__ == "__main__":
    run_benchmark("laplacian", laplacian)
