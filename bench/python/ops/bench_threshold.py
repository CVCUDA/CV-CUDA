...
"""CV-CUDA Threshold operator benchmark - Python equivalent of BenchThreshold.cpp"""
...
def threshold(state):
    """Threshold operator benchmark matching C++ BenchThreshold.cpp"""
    ...
    if dtype_str in ("uint8", "U8") and num_channels == 1:
        thresh_type = cvcuda.ThresholdType.BINARY | cvcuda.ThresholdType.OTSU
    else:
        thresh_type = cvcuda.ThresholdType.BINARY

    if layout not in ("NHWC", "NCHW", "NCHW_FAKE"):
        state.skip(
            "Threshold benchmark supports only NHWC, NCHW, and NCHW_FAKE layouts"
        )
        return None
    ...
    if is_planar and input_kind == "VarShape" and num_channels == 1:
        state.skip(
            "Single-channel varshape Threshold has no distinct planar image layout"
        )
        return None
    ...
    thresh = create_tensor((N,), "float64", device_id, layout="N", fill_mode="lcg")
    maxval = create_tensor((N,), "float64", device_id, layout="N", fill_mode="lcg")
    ...
    if is_fake_planar:
        ...
        def run_fake(launch):
            ...
            cvcuda.threshold_into(
                inter_dst, inter_src, thresh, maxval, thresh_type, stream=stream
            )
    ...
    if input_kind == "Tensor":
        ...
        def run(launch):
            stream = get_stream(launch)
            cvcuda.threshold_into(dst, src, thresh, maxval, thresh_type, stream=stream)
