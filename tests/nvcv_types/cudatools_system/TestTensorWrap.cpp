...
TYPED_TEST(Tensor2DWrapImageTest, correct_with_image)
{
    ...
    auto ptr0 = reinterpret_cast<const ValueType *>(dev->plane(0).basePtr);
    auto ptr1 = reinterpret_cast<const ValueType *>(dev->plane(0).basePtr + dev->plane(0).rowStride);
...
{
    const auto &tensorShape    = ::nvcv::test::ParamValue(std::get<0>(GetParam()));
    const auto &tensorDataType = ::nvcv::test::ParamValue(std::get<1>(GetParam()));
