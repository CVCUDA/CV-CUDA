...
def test_imgbatchvarshape_creation_works():
    batch = cvcuda.ImageBatchVarShape(15)
...
def test_imgbatchvarshape_one_image():
    ...
    img = cvcuda.Image((64, 32), cvcuda.Format.RGBA8)
    batch.pushback(img)
    assert len(batch) == 1
...
def test_imgbatchvarshape_several_images():
    ...
    imgs = [cvcuda.Image((m * 2, m), cvcuda.Format.RGBA8) for m in range(2, 10, 2)]
    batch.pushback(imgs)
    assert len(batch) == 4
    ...
    batch.pushback(cvcuda.Image((58, 26), cvcuda.Format.NV12))
...
def test_wrap_buffer_list(base_shape, dt, format):
    ...
    batch = cvcuda.as_images(cuda_buffers)
